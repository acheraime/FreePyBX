...
<<<<<<< HEAD
db_connection = assert(env:connect("dbname=pbx user=pybx password=3v3lyn555 host=127.0.0.1"))
=======
db_connection = assert(env:connect("dbname=freepybx user=freepybx password=secretpass1 host=127.0.0.1"))
...
while attempt <= max_attempts do
    ...
    db_cursor = assert(db_connection:execute(string.format("select pin from pbx_conference_bridges where extension = '%s' and context='%s'", tostring(conf_num), context)))
    ...
    and invalid conference number ]]--
        session:speak("I am sorry, you have entered an incorrect pin.")
    ...
    elseif row["pin"] == nil or row["pin"] == "" then
        ...
        if ((get_conference_pin(1, 4, 3, 4000, row["pin"])) == true) then
            ...
            session:execute("conference", string.format("%s@sip.ipcomms.net", conf_num))
        else
