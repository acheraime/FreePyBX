...
"CREATE TABLE call_center_callers (\n"
"   id           serial NOT NULL,\n"
"   queue	     VARCHAR(255),\n"
"   system	     VARCHAR(255),\n"
...
"CREATE TABLE call_center_tiers (\n"
"   id       serial NOT NULL,\n"
"   queue    VARCHAR(255),\n"
"   agent    VARCHAR(255),\n"
"   state    VARCHAR(255),\n"
...
int cc_queue_count(const char *queue)
{
	...
	if (!switch_strlen_zero(queue)) {
		if (queue[0] == '*') {
			...
			sql = switch_mprintf("SELECT count(*) FROM call_center_callers WHERE state = '%q' OR state = '%q'",
					cc_member_state2str(CC_MEMBER_STATE_WAITING), cc_member_state2str(CC_MEMBER_STATE_TRYING));
		} else {
			sql = switch_mprintf("SELECT count(*) FROM call_center_callers WHERE queue = '%q' AND (state = '%q' OR state = '%q')",
...
cc_status_t cc_agent_add(const char *agent, const char *type)
{
	...
	if (!strcasecmp(type, CC_AGENT_TYPE_CALLBACK) || !strcasecmp(type, CC_AGENT_TYPE_UUID_STANDBY)) {
		...
		sql = switch_mprintf("SELECT count(*) FROM call_center_agents WHERE name = '%q'", agent);
		cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
		switch_safe_free(sql);
		...
		sql = switch_mprintf("INSERT INTO call_center_agents (name, system, type, status, state) VALUES('%q', 'single_box', '%q', '%q', '%q');", 
...
cc_status_t cc_agent_del(const char *agent)
{
	...
	sql = switch_mprintf("DELETE FROM call_center_agents WHERE name = '%q';"
			"DELETE FROM call_center_tiers WHERE agent = '%q';",
			agent, agent);
	cc_execute_sql(NULL, sql, NULL);
	switch_safe_free(sql);
	return result;
...
cc_status_t cc_agent_get(const char *key, const char *agent, char *ret_result, size_t ret_result_size)
{
	...
	if (!strcasecmp(key, "status") || !strcasecmp(key, "state") || !strcasecmp(key, "uuid") ) { 
		/* Check to see if agent already exists */
		sql = switch_mprintf("SELECT %q FROM call_center_agents WHERE name = '%q'", key, agent);
		cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
		switch_safe_free(sql);
	...
	if (!strcasecmp(key, "status")) {
		if (cc_agent_str2status(value) != CC_AGENT_STATUS_UNKNOWN) {
			...
			if (cc_agent_str2status(value) == CC_AGENT_STATUS_AVAILABLE) {
				sql = switch_mprintf("UPDATE call_center_agents SET status = '%q', last_status_change = '%" SWITCH_TIME_T_FMT "', talk_time = 0, calls_answered = 0, no_answer_count = 0"
						" WHERE name = '%q' AND NOT status = '%q'",
						value, local_epoch_time_now(NULL),
						agent, value);
			} else {
				sql = switch_mprintf("UPDATE call_center_agents SET status = '%q', last_status_change = '%" SWITCH_TIME_T_FMT "' WHERE name = '%q'",
			...
			if (cc_agent_str2status(value) != CC_AGENT_STATUS_AVAILABLE) {
				sql = switch_mprintf("SELECT uuid FROM call_center_callers WHERE serving_agent = '%q' AND serving_system = 'single_box' AND NOT state = 'Answered'", agent);
				cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
				switch_safe_free(sql);
		...
		if (cc_agent_str2state(value) != CC_AGENT_STATE_UNKNOWN) {
			if (cc_agent_str2state(value) != CC_AGENT_STATE_RECEIVING) {
				sql = switch_mprintf("UPDATE call_center_agents SET state = '%q' WHERE name = '%q'", value, agent);
			} else {
				sql = switch_mprintf("UPDATE call_center_agents SET state = '%q', last_offered_call = '%" SWITCH_TIME_T_FMT "' WHERE name = '%q'",
						value, local_epoch_time_now(NULL), agent);
			}
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE call_center_agents SET uuid = '%q', system = 'single_box' WHERE name = '%q'", value, agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);

		result = CC_STATUS_SUCCESS;
	} else if (!strcasecmp(key, "contact")) {
		sql = switch_mprintf("UPDATE call_center_agents SET contact = '%q', system = 'single_box' WHERE name = '%q'", value, agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);

		result = CC_STATUS_SUCCESS;
	} else if (!strcasecmp(key, "ready_time")) {
		sql = switch_mprintf("UPDATE call_center_agents SET ready_time = '%ld', system = 'single_box' WHERE name = '%q'", atol(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);

		result = CC_STATUS_SUCCESS;
	} else if (!strcasecmp(key, "busy_delay_time")) {
		sql = switch_mprintf("UPDATE call_center_agents SET busy_delay_time = '%ld', system = 'single_box' WHERE name = '%q'", atol(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);

		result = CC_STATUS_SUCCESS;
	} else if (!strcasecmp(key, "reject_delay_time")) {
		sql = switch_mprintf("UPDATE call_center_agents SET reject_delay_time = '%ld', system = 'single_box' WHERE name = '%q'", atol(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);

		result = CC_STATUS_SUCCESS;
	} else if (!strcasecmp(key, "no_answer_delay_time")) {
		sql = switch_mprintf("UPDATE call_center_agents SET no_answer_delay_time = '%ld', system = 'single_box' WHERE name = '%q'", atol(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE call_center_agents SET type = '%q' WHERE name = '%q'", value, agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE call_center_agents SET max_no_answer = '%d', system = 'single_box' WHERE name = '%q'", atoi(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE call_center_agents SET wrap_up_time = '%d', system = 'single_box' WHERE name = '%q'", atoi(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
...
cc_status_t cc_tier_add(const char *queue_name, const char *agent, const char *state, int level, int position)
{
	...
	if (cc_tier_str2state(state) != CC_TIER_STATE_UNKNOWN) {
		...
		sql = switch_mprintf("SELECT count(*) FROM call_center_tiers WHERE agent = '%q' AND queue = '%q'", agent, queue_name);
		cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
		switch_safe_free(sql);
		...
		sql = switch_mprintf("INSERT INTO call_center_tiers (queue, agent, state, level, position) VALUES('%q', '%q', '%q', '%d', '%d');",
...
cc_status_t cc_tier_update(const char *key, const char *value, const char *queue_name, const char *agent)
{
	...
	if (!strcasecmp(key, "state")) {
		if (cc_tier_str2state(value) != CC_TIER_STATE_UNKNOWN) {
			sql = switch_mprintf("UPDATE call_center_tiers SET state = '%q' WHERE queue = '%q' AND agent = '%q'", value, queue_name, agent);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
			result = CC_STATUS_SUCCESS;
		} else {
		...
		sql = switch_mprintf("UPDATE call_center_tiers SET level = '%d' WHERE queue = '%q' AND agent = '%q'", atoi(value), queue_name, agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE call_center_tiers SET position = '%d' WHERE queue = '%q' AND agent = '%q'", atoi(value), queue_name, agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
...
cc_status_t cc_tier_del(const char *queue_name, const char *agent)
{
	...
	sql = switch_mprintf("DELETE FROM call_center_tiers WHERE queue = '%q' AND agent = '%q';", queue_name, agent);
	cc_execute_sql(NULL, sql, NULL);
	switch_safe_free(sql);
...
static switch_status_t load_config(void)
{
	...
	switch_cache_db_test_reactive(dbh, "select count(session_uuid) from  call_center_callers", "drop table  call_center_callers", members_sql);
	switch_cache_db_test_reactive(dbh, "select count(ready_time) from  call_center_agents", NULL, "alter table call_center_agents add ready_time integer not null default 0;"
									   "alter table call_center_agents add reject_delay_time integer not null default 0;"
									   "alter table call_center_agents add busy_delay_time  integer not null default 0;");
	switch_cache_db_test_reactive(dbh, "select count(no_answer_delay_time) from  call_center_agents", NULL, "alter table call_center_agents add no_answer_delay_time integer not null default 0;");
	switch_cache_db_test_reactive(dbh, "select count(ready_time) from  call_center_agents", "drop table  call_center_agents", agents_sql);
	switch_cache_db_test_reactive(dbh, "select count(queue) from  call_center_tiers", "drop table  call_center_tiers" , tiers_sql);

	switch_cache_db_release_db_handle(&dbh);

	/* Reset a unclean shutdown */
	sql = switch_mprintf("update call_center_agents set state = 'Waiting', uuid = '' where system = 'single_box';"
						 "update call_center_tiers set state = 'Ready' where agent IN (select name from call_center_agents where system = 'single_box');"
						 "update call_center_callers set state = '%q', session_uuid = '' where system = 'single_box';",
	...
	/* Importing from XML config call_center_agents */
	if ((x_agents = switch_xml_child(cfg, "agents"))) {
	...
	/* Importing from XML config Agent call_center_tiers */
...
static void *SWITCH_THREAD_FUNC outbound_agent_thread_run(switch_thread_t *thread, void *obj)
{
	...
	if (!member_session) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Member %s <%s> with uuid %s in queue %s is gone just before we assigned an agent\n", h->member_cid_name, h->member_cid_number, h->member_session_uuid, h->queue_name);
		 ...
		 sql = switch_mprintf("UPDATE call_center_callers SET state = '%q', session_uuid = '', abandoned_epoch = '%" SWITCH_TIME_T_FMT "' WHERE system = 'single_box' AND uuid = '%q' AND state != '%q'",
	...
	if (status == SWITCH_STATUS_SUCCESS) {
		...
		*/
		...
		if (!strcasecmp(h->queue_strategy,"ring-all")) {
			...
			sql = switch_mprintf("UPDATE call_center_callers SET serving_agent = '%q', serving_system = 'single_box', state = '%q'"
			...
			sql = switch_mprintf("SELECT count(*) FROM  call_center_callers"
		...
		sql = switch_mprintf("UPDATE call_center_agents SET uuid = '%q', last_bridge_start = '%" SWITCH_TIME_T_FMT "', calls_answered = calls_answered + 1, no_answer_count = 0"
		...
		/* Change the call_center_agents Status in the call_center_tiers */
		...
		/* Record session if record-template is provided and agent is set to record_calls=1*/

		sql = switch_mprintf("SELECT record_calls FROM call_center_agents WHERE name = '%q'", h->agent_name);
		cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
		switch_safe_free(sql);

		if (h->record_template || (atoi(res) != 0)) {
		...
		/* Update call_center_agents Items */
		/* Do not remove uuid of the agent if we are a standby agent */
		sql = switch_mprintf("UPDATE call_center_agents SET %s last_bridge_end = %" SWITCH_TIME_T_FMT ", talk_time = talk_time + (%" SWITCH_TIME_T_FMT "-last_bridge_start) WHERE name = '%q' AND system = '%q';"
		...
		sql = switch_mprintf("DELETE FROM call_center_callers WHERE system = 'single_box' AND uuid = '%q'", h->member_uuid);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE call_center_callers SET state = '%q', serving_agent = '', serving_system = ''"
		...
		switch (cause) {
			/* When we hang-up call_center_agents that did not answer in ring-all strategy */
			...
			default:
				...
				sql = switch_mprintf("UPDATE call_center_agents SET no_answer_count = no_answer_count + 1 WHERE name = '%q' AND system = '%q';",
						h->agent_name, h->agent_system);
				cc_execute_sql(NULL, sql, NULL);
				switch_safe_free(sql);

				/* Insert Agent Log */
				sql = switch_mprintf("INSERT INTO call_center_agent_logs (agent,queue,log_epoch,log_type,data, uuid) VALUES ('%q','%q',%d, 1,'Agent passed on call.', '%q');",
						h->agent_name, h->queue_name, local_epoch_time_now(NULL), h->member_uuid);
				cc_execute_sql(NULL, sql, NULL);
				switch_safe_free(sql);
	...
	sql = switch_mprintf(
			"UPDATE call_center_tiers SET state = '%q' WHERE agent = '%q' AND queue = '%q' AND (state = '%q' OR state = '%q' OR state = '%q');"
			"UPDATE call_center_tiers SET state = '%q' WHERE agent = '%q' AND NOT queue = '%q' AND state = '%q'"
...
static int agents_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	if (strcasecmp(agent_system,"single_box" /* SELF */)) {
		if (!strcasecmp(cbt->strategy, "ring-all")) {
			...
			return 0; /* Skip this call_center_agents only, so we can ring the other one */
		}
	}

	if (!strcasecmp(cbt->strategy,"ring-all")) {
		/* Check if member is a ring-all mode */
		sql = switch_mprintf("SELECT count(*) FROM call_center_callers WHERE serving_agent = 'ring-all' AND uuid = '%q' AND system = 'single_box'", cbt->member_uuid);
		cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
		...
		/* Check if we won the race to get the member to our selected agent (Used for Multi system purposes) */
		sql = switch_mprintf("SELECT count(*) FROM call_center_callers WHERE serving_agent = '%q' AND serving_system = 'single_box' AND uuid = '%q' AND system = 'single_box'",
		...
		default: /* Go ahead, start thread to try to bridge these 2 caller */
			{
				...
				sql = switch_mprintf(
						"UPDATE call_center_tiers SET state = '%q' WHERE agent = '%q' AND queue = '%q';"
						"UPDATE call_center_tiers SET state = '%q' WHERE agent = '%q' AND NOT queue = '%q' AND state = '%q';",
...
static int members_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	if (!strcasecmp(member_state, cc_member_state2str(CC_MEMBER_STATE_ABANDONED))) {
		...
		if (abandoned_epoch + discard_abandoned_after < local_epoch_time_now(NULL)) {
			sql = switch_mprintf("DELETE FROM call_center_callers WHERE system = 'single_box' AND uuid = '%q' AND (abandoned_epoch = '%" SWITCH_TIME_T_FMT "' OR joined_epoch = '%q')", cbt.member_uuid, abandoned_epoch, cbt.member_joined_epoch);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
		}
		/* Skip this member */
	...
	if (!strcasecmp(queue->strategy, "top-down")) {
		...
		sql = switch_mprintf("SELECT system, name, status, contact, no_answer_count, max_no_answer, reject_delay_time, busy_delay_time, no_answer_delay_time,  call_center_tiers.state,  call_center_agents.last_bridge_end,  call_center_agents.wrap_up_time,  call_center_agents.state,  call_center_agents.ready_time,  call_center_tiers.position as tiers_position,  call_center_tiers.level as tiers_level,  call_center_agents.type,  call_center_agents.uuid,  call_center_agents.last_offered_call as agents_last_offered_call, 1 as dyn_order FROM call_center_agents LEFT JOIN call_center_tiers ON (call_center_agents.name =  call_center_tiers.agent)"
				" WHERE  call_center_tiers.queue = '%q'"
				" AND (call_center_agents.status = '%q' OR  call_center_agents.status = '%q' OR  call_center_agents.status = '%q')"
				" AND  call_center_tiers.position > %d"
				" AND  call_center_tiers.level = %d"
				" UNION "
				"SELECT system, name, status, contact, no_answer_count, max_no_answer, reject_delay_time, busy_delay_time, no_answer_delay_time,  call_center_tiers.state,  call_center_agents.last_bridge_end,  call_center_agents.wrap_up_time,  call_center_agents.state,  call_center_agents.ready_time,  call_center_tiers.position as tiers_position,  call_center_tiers.level as tiers_level,  call_center_agents.type,  call_center_agents.uuid,  call_center_agents.last_offered_call as agents_last_offered_call, 2 as dyn_order FROM call_center_agents LEFT JOIN call_center_tiers ON (call_center_agents.name =  call_center_tiers.agent)"
				" WHERE  call_center_tiers.queue = '%q'"
				" AND (call_center_agents.status = '%q' OR  call_center_agents.status = '%q' OR  call_center_agents.status = '%q')"
		...
		sql = switch_mprintf("SELECT system, name, status, contact, no_answer_count, max_no_answer, reject_delay_time, busy_delay_time, no_answer_delay_time,  call_center_tiers.state,  call_center_agents.last_bridge_end,  call_center_agents.wrap_up_time,  call_center_agents.state,  call_center_agents.ready_time,  call_center_tiers.position as tiers_position,  call_center_tiers.level as tiers_level,  call_center_agents.type,  call_center_agents.uuid,  call_center_agents.last_offered_call as agents_last_offered_call, 1 as dyn_order FROM call_center_agents LEFT JOIN call_center_tiers ON (call_center_agents.name =  call_center_tiers.agent)"
				" WHERE  call_center_tiers.queue = '%q'"
				" AND (call_center_agents.status = '%q' OR  call_center_agents.status = '%q' OR  call_center_agents.status = '%q')"
				" AND  call_center_tiers.position > (SELECT  call_center_tiers.position FROM call_center_agents LEFT JOIN call_center_tiers ON (call_center_agents.name =  call_center_tiers.agent) WHERE  call_center_tiers.queue = '%q' AND  call_center_agents.last_offered_call > 0 ORDER BY  call_center_agents.last_offered_call DESC LIMIT 1)"
				" AND  call_center_tiers.level = (SELECT  call_center_tiers.level FROM call_center_agents LEFT JOIN call_center_tiers ON (call_center_agents.name =  call_center_tiers.agent) WHERE  call_center_tiers.queue = '%q' AND  call_center_agents.last_offered_call > 0 ORDER BY  call_center_agents.last_offered_call DESC LIMIT 1)"
				" UNION "
				"SELECT system, name, status, contact, no_answer_count, max_no_answer, reject_delay_time, busy_delay_time, no_answer_delay_time,  call_center_tiers.state,  call_center_agents.last_bridge_end,  call_center_agents.wrap_up_time,  call_center_agents.state,  call_center_agents.ready_time,  call_center_tiers.position as tiers_position,  call_center_tiers.level as tiers_level,  call_center_agents.type,  call_center_agents.uuid,  call_center_agents.last_offered_call as agents_last_offered_call, 2 as dyn_order FROM call_center_agents LEFT JOIN call_center_tiers ON (call_center_agents.name =  call_center_tiers.agent)"
				" WHERE  call_center_tiers.queue = '%q'"
				" AND (call_center_agents.status = '%q' OR  call_center_agents.status = '%q' OR  call_center_agents.status = '%q')"
		...
		if (!strcasecmp(queue->strategy, "longest-idle-agent")) {
			sql_order_by = switch_mprintf("level,  call_center_agents.last_offered_call, position");
		} else if (!strcasecmp(queue_strategy, "agent-with-least-talk-time")) {
			sql_order_by = switch_mprintf("level,  call_center_agents.talk_time, position");
		} else if (!strcasecmp(queue_strategy, "agent-with-fewest-calls")) {
			sql_order_by = switch_mprintf("level,  call_center_agents.calls_answered, position");
		} else if (!strcasecmp(queue_strategy, "ring-all")) {
			sql = switch_mprintf("UPDATE call_center_callers SET state = '%q' WHERE state = '%q' AND uuid = '%q' AND system = 'single_box'",
			...
			sql_order_by = switch_mprintf("level, position,  call_center_agents.last_offered_call"); /* Default to last_offered_call, let add new strategy if needing it differently */
		} else {
			/* If the strategy doesn't exist, just fallback to the following */
			sql_order_by = switch_mprintf("level, position,  call_center_agents.last_offered_call");
		}

		sql = switch_mprintf("SELECT system, name, status, contact, no_answer_count, max_no_answer, reject_delay_time, busy_delay_time, no_answer_delay_time,  call_center_tiers.state,  call_center_agents.last_bridge_end,  call_center_agents.wrap_up_time,  call_center_agents.state,  call_center_agents.ready_time,  call_center_tiers.position,  call_center_tiers.level,  call_center_agents.type,  call_center_agents.uuid FROM call_center_agents LEFT JOIN call_center_tiers ON (call_center_agents.name =  call_center_tiers.agent)"
				" WHERE  call_center_tiers.queue = '%q'"
				" AND (call_center_agents.status = '%q' OR  call_center_agents.status = '%q' OR  call_center_agents.status = '%q')"
				" ORDER BY %q",
				queue_name,
...
void *SWITCH_THREAD_FUNC cc_agent_dispatch_thread_run(switch_thread_t *thread, void *obj)
{
	...
	while (globals.running == 1) {
		char *sql = NULL;
		sql = switch_mprintf("SELECT queue,uuid,session_uuid,cid_number,cid_name,joined_epoch,(%" SWITCH_TIME_T_FMT "-joined_epoch)+base_score+skill_score AS score, state, abandoned_epoch FROM  call_center_callers"
...
SWITCH_STANDARD_APP(callcenter_function)
{
	...
	if (queue->abandoned_resume_allowed == SWITCH_TRUE) {
		...
		/* Check to see if agent already exist */
		sql = switch_mprintf("SELECT uuid FROM call_center_callers WHERE queue = '%q' AND cid_number = '%q' AND state = '%q' ORDER BY abandoned_epoch DESC",
		...
		if (!zstr(member_uuid)) {
			sql = switch_mprintf("SELECT abandoned_epoch FROM call_center_callers WHERE uuid = '%q'", member_uuid);
			cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
			switch_safe_free(sql);
	...
	if (abandoned_epoch == 0) {
		...
		sql = switch_mprintf("INSERT INTO  call_center_callers"
		...
		sql = switch_mprintf("UPDATE call_center_callers SET session_uuid = '%q', state = '%q', rejoined_epoch = '%" SWITCH_TIME_T_FMT "' WHERE uuid = '%q' AND state = '%q'",
		...
		sql = switch_mprintf("SELECT abandoned_epoch FROM call_center_callers WHERE uuid = '%q' AND session_uuid = '%q' AND state = '%q' AND queue = '%q'", member_uuid, member_session_uuid, cc_member_state2str(CC_MEMBER_STATE_WAITING), queue_name);
		cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
		switch_safe_free(sql);
	...
	if (!switch_channel_up(member_channel) || h->member_cancel_reason != CC_MEMBER_CANCEL_REASON_NONE) {
		...
		/* Update member state */
		sql = switch_mprintf("UPDATE call_center_callers SET state = '%q', session_uuid = '', abandoned_epoch = '%" SWITCH_TIME_T_FMT "' WHERE system = 'single_box' AND uuid = '%q'",
				cc_member_state2str(CC_MEMBER_STATE_ABANDONED), local_epoch_time_now(NULL), member_uuid);
				cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);

		/* Hangup any callback call_center_agents  */
		...
		/* Update member state */
		sql = switch_mprintf("UPDATE call_center_callers SET state = '%q', bridge_epoch = '%" SWITCH_TIME_T_FMT "' WHERE system = 'single_box' AND uuid = '%q'",
...
"\tcallcenter_config queue list call_center_agents [queue_name] [status] | \n" \
"\tcallcenter_config queue list call_center_callers [queue_name] | \n" \
"\tcallcenter_config queue list call_center_tiers [queue_name] | \n" \
"\tcallcenter_config queue count | \n" \
"\tcallcenter_config queue count call_center_agents [queue_name] [status] | \n" \
"\tcallcenter_config queue count call_center_callers [queue_name] | \n" \
"\tcallcenter_config queue count call_center_tiers [queue_name]"
	...
	if (section && !strcasecmp(section, "agent")) {
		if (action && !strcasecmp(action, "add")) {
			...
			if ( argc-initial_argc > 1 ) {
				...
				sql = switch_mprintf("SELECT * FROM call_center_agents WHERE name='%q'", argv[0 + initial_argc]);
			} else {
				sql = switch_mprintf("SELECT * FROM  call_center_agents");
			}
		...
		if (action && !strcasecmp(action, "add")) {
			...
			cbt.row_process = 0;
			cbt.stream = stream;
			sql = switch_mprintf("SELECT * FROM call_center_tiers ORDER BY level, position");
			cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, list_result_callback, &cbt /* Call back variables */);
			switch_safe_free(sql);
			stream->write_function(stream, "%s", "+OK\n");
		}
		...
		if (action && !strcasecmp(action, "load")) {
			...
			if (argc-initial_argc < 1) {
				...
				/* queue list call_center_agents */
				if (sub_action && !strcasecmp(sub_action, "call_center_agents")) {
					if (argc-initial_argc > 2) {
						status = argv[2 + initial_argc];
					}
					if (status)	{
						sql = switch_mprintf("SELECT  call_center_agents.* FROM   call_center_agents,call_center_tiers WHERE  call_center_tiers.agent =  call_center_agents.name AND  call_center_tiers.queue = '%q' AND  call_center_agents.status = '%q'", queue_name, status);
					} else {
						sql = switch_mprintf("SELECT  call_center_agents.* FROM   call_center_agents,call_center_tiers WHERE  call_center_tiers.agent =  call_center_agents.name AND  call_center_tiers.queue = '%q'", queue_name);
					}
				/* queue list call_center_callers */
				} else if (sub_action && !strcasecmp(sub_action, "call_center_callers")) {
					sql = switch_mprintf("SELECT * FROM call_center_callers WHERE queue = '%q';", queue_name);
				/* queue list call_center_tiers */
				} else if (sub_action && !strcasecmp(sub_action, "call_center_tiers")) {
					sql = switch_mprintf("SELECT * FROM call_center_tiers WHERE queue = '%q';", queue_name);
				} else {
					stream->write_function(stream, "%s", "-ERR Invalid!\n");
					goto done;
				}
			...
			if (argc-initial_argc < 1) {
				...
				/* queue count call_center_agents */
				if (sub_action && !strcasecmp(sub_action, "call_center_agents")) {
					if (argc-initial_argc > 2) {
						status = argv[2 + initial_argc];
					}
					if (status)	{
						sql = switch_mprintf("SELECT count(*) FROM   call_center_agents,call_center_tiers WHERE  call_center_tiers.agent =  call_center_agents.name AND  call_center_tiers.queue = '%q' AND  call_center_agents.status = '%q'", queue_name, status);
					} else {
						sql = switch_mprintf("SELECT count(*) FROM   call_center_agents,call_center_tiers WHERE  call_center_tiers.agent =  call_center_agents.name AND  call_center_tiers.queue = '%q'", queue_name);
					}
				/* queue count call_center_callers */
				} else if (sub_action && !strcasecmp(sub_action, "call_center_callers")) {
					sql = switch_mprintf("SELECT count(*) FROM call_center_callers WHERE queue = '%q';", queue_name);
				/* queue count call_center_tiers */
				} else if (sub_action && !strcasecmp(sub_action, "call_center_tiers")) {
					sql = switch_mprintf("SELECT count(*) FROM call_center_tiers WHERE queue = '%q';", queue_name);
				} else {
					stream->write_function(stream, "%s", "-ERR Invalid!\n");
					goto done;
				}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_callcenter_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	switch_console_set_complete("add callcenter_config queue list  call_center_agents");
	switch_console_set_complete("add callcenter_config queue list  call_center_callers");
	switch_console_set_complete("add callcenter_config queue list  call_center_tiers");
	switch_console_set_complete("add callcenter_config queue count");
	switch_console_set_complete("add callcenter_config queue count  call_center_agents");
	switch_console_set_complete("add callcenter_config queue count  call_center_callers");
	switch_console_set_complete("add callcenter_config queue count  call_center_tiers");
